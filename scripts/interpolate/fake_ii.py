...
def gp_des_lc(data):
    ...
    df.to_sql('fake_ii_interp_46', engine, if_exists='append', index=False)


query = "SELECT DISTINCT snid FROM fake_ii_obs"
snid_list = des.query_localdb(query)['snid'].values

for i, snid in enumerate(snid_list):
    print(i, snid)

    query = "SELECT * FROM fake_ii_obs WHERE snid={}".format(int(snid))
